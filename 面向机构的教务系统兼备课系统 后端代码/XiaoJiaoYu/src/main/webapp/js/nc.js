		...
		NEW_ACTION = function() {
			! function(e) {
					...
					return NoCaptcha.prototype = {
						...
						inituab: function() {
							...
							UA_Opt.Token = (new Date).getTime() + ":" + opt.token, _.loadjs(util.addHourStamp(ajaxURL.uab_Url), function(t) {
								if("timeout" === t) e.awscTimeout = 1, showError(!0, ERR_CODE_UABTIMEOUT), report("loaduab failed"), reportLoadJSError(LOAD_JS_TIMEOUT, "uab.js timeout");
								else try {
									var n = function() {
						...
						afterUA: function() {
							"undefined" == typeof umx ? this.initUM() : (umx.init({
								timeout: opt.timeout,
								token: opt.token,
						...
						initUM: function() {
							function e() {
								t.awscTimeout ? clearInterval(n) : window.AWSC && (clearInterval(n), window.AWSC.use("um", function(e, n) {
									try {
										if("loaded" === e) umx.init({
											timeout: opt.timeout,
											token: opt.token,
											serviceUrl: ajaxURL.umid_serUrl,
											appName: opt.appkey,
										...
										else if("timeout" === e) showError(!0, ERR_CODE_UMTIMEOUT), reportLoadJSError(LOAD_JS_TIMEOUT, "um.js timeout");
										else {
						...
						onScaleReadyCallback: function(e, t) {
							if(e.success) {
								var n = e.result,
									i = n.code;
								if(0 === i) _.id(scale_btn).className = "nc_iconfont btn_ok", _.id(scale_btn).innerHTML = icon_ok_sign, TEXTELEM.innerHTML = language[opt.language]._yesTEXT, util.removeClass(t.btn.parentNode, "nc_err"), this.userCallback(n.csessionid, "pass", n.value);
								else if(UA_Opt.reload && (UA_Opt.Token = (new Date).getTime() + ":" + opt.token, UA_Opt.reload && UA_Opt.reload()), util.addClass(t.btn, "nc_iconfont btn_warn"), util.addClass(t.btn.parentNode, "nc_err"), t.btn.innerHTML = icon_warn, t.bar = _.tag(scale_bar + " div")[0], TEXTELEM.innerHTML = language[opt.language]._Loading + loading_circle_html, "function" == typeof opt.verifycallback && 300 != i && opt.verifycallback(n), 100 == i) this.__inn = 1, this.onScale100(n.csessionid, n.value);
								else if(200 == i) this.__inn = 1, this.onScale200(n.csessionid, n.value);
								else if(260 == i) this.onScale260(n.csessionid, n.value);
				...
				function i(e, t, n) {
					...
					var a = void 0,
						r = l[e.foreign] || l[0];
					r = s.mix(r, e.apimap), UA_Opt.LogVal = "_n", d.init(e), UA_Opt.sendMethod = 8, UA_Opt.Token = (new Date).getTime() + ":" + e.token, s.loadScript(s.addHourStamp(r.uab_Url), function(e) {
						if(AWSC) {
							var t = function() {
								if(window.__sc__uab) {
									if(clearInterval(n), "timeout" === window.__sc__uab) return void i(u);
									i(e)
								}
							};
